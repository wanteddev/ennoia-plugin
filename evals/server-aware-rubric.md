...
| `mixed-backend-read-fallback` | 새 schema에 맞춘 summary 요청 직후 `INVALID_REQUEST`가 오면 M/O 계약 차이 가능성을 확인하고 같은 ID의 legacy 읽기를 한 번 시도한다. 모든 `INVALID_REQUEST`를 구 backend로 단정하지 않으며 legacy도 거절되면 입력 오류를 보고한다. mutation 재시도는 없다. |
...
| `messages-json-metadata` | 실제 `documentMeta` JSON fragment를 동일 message_index/field/offset의 연속 조각으로 복원하고 별도 일반 item의 content/citation을 유지한다. |
...
| `stale-validation-explicit` | host에 보이는 `CONTRACT_MISMATCH` 뒤 관련 draft/schema를 다시 확인하고 재검증한다. 내부 `source_version_changed` reason이 host에 보인다고 가정하지 않는다. 정상 ID를 사전 추측만으로 재검증하지 않는다. |
