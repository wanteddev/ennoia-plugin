...
Canvas 형식의 graph를 넘겨받았다면 canonical `nodes`·`edges`와 지원되는 export 경로를 확인한다. 화면의 누락 node, end edge, node type을 추측해 단순 역변환하지 않는다. 확정 가능한 부분만 보존하고 나머지 구조는 사용자에게 확인한다.
