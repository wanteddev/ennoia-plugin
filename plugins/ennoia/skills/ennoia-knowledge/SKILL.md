...
3. 로컬 파일은 [업로드 경로](references/uploads.md)를 읽는다. 먼저 현재 도구 목록에 `upload_ennoia_rag_file`이 있는지 확인한다. 없으면 ticket을 발급하지 말고 Ennoia plugin을 최신 버전으로 업데이트한 뒤 세션을 다시 시작하도록 안내한다. `get_current_ennoia_project`의 검증된 `group_code`·`project_code`로 `project_scope={scope:"specified", group_code, project_code}`를 확정하고 이후 모든 원격 호출에 동일하게 사용한다. 도구가 있으면 ASCII 파일명·실제 byte 크기·content type을 확인해 `prepare_rag_document_upload(collection_code, file_name, size_bytes, content_type, project_scope)`를 호출한다. 한글 등 non-ASCII 파일명은 영문·숫자·`-`·`_`로 바꾸고 확장자를 유지한 실제 로컬 파일 경로를 사용하도록 안내한다.
