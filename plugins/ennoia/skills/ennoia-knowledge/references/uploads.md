...
8. 웹 링크는 확인된 `group_code`, `project_code`, `collection_code`를 각각 percent-encoding하여 만든다. 표시 이름이나 예시 ID를 대신 넣지 않는다. 운영 upload URL host가 `mcp.ennoia.so`이면 `[Ennoia에서 파일 업로드](https://ennoia.so/studio/rag/files-detail?group={group_code}&project={project_code}&slug=rag&collection={collection_code})`, dev host가 `dev-mcp-server.ennoia.so`이면 `[Ennoia dev에서 파일 업로드](https://dev.ennoia.so/studio/rag/files-detail?group={group_code}&project={project_code}&slug=rag&collection={collection_code})` 형식을 사용한다. 결과에는 `{...}` placeholder가 아니라 실제 값을 넣은 클릭 가능한 Markdown 링크를 제공한다. 현재 context에 code가 없으면 `get_current_ennoia_project`로 확인하고, 그래도 확인되지 않으면 잘못된 링크를 만들지 않는다.
